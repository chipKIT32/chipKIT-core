...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _OSC_OSC_SYS_CLK_DIVISOR_VREG(index)

  MASKs: 
    _OSC_OSC_SYS_CLK_DIVISOR_MASK(index)

  POSs: 
    _OSC_OSC_SYS_CLK_DIVISOR_POS(index)

  LENs: 
    _OSC_OSC_SYS_CLK_DIVISOR_LEN(index)
...
{
    _SFR_FIELD_WRITE(_OSC_OSC_SYS_CLK_DIVISOR_VREG(index),
						 _OSC_OSC_SYS_CLK_DIVISOR_MASK(index),
						 _OSC_OSC_SYS_CLK_DIVISOR_POS(index) ,
						  (systemClkDivisor-1) );
}
...
{
    return (_SFR_FIELD_READ(_OSC_OSC_SYS_CLK_DIVISOR_VREG(index),
						 _OSC_OSC_SYS_CLK_DIVISOR_MASK(index),
						 _OSC_OSC_SYS_CLK_DIVISOR_POS(index) )+1);
}
