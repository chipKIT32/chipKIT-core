...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _OSC_FRC_READY_VREG(index)
    _OSC_PRIMARY_READY_VREG(index)

  MASKs: 
    _OSC_FRC_READY_MASK(index)
    _OSC_PRIMARY_READY_MASK(index)

  POSs: 
    _OSC_FRC_READY_POS(index)
    _OSC_PRIMARY_READY_POS(index)

  LENs: 
    _OSC_FRC_READY_LEN(index)
    _OSC_PRIMARY_READY_LEN(index)
...
{
    return _SFR_BIT_READ(_OSC_FRC_READY_VREG(index),
                             (_OSC_FRC_READY_POS(index) + clk));
}
